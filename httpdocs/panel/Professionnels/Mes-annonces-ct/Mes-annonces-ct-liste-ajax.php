...
if (!empty($_SESSION['4M8e7M5b1R2e8s']) && !empty($user)) {
  ...
  $nom_fichier = "Mes-annonces-ct";
  $nom_fichier_datatable = "Mes-annonces-ct-" . date('d-m-Y', time()) . "-$nomsiteweb";
?>
  <script>
  ...
  <table id='Tableau_7' class='display' style='text-align: center; width: 100%; margin-top: 15px;' cellpadding='2' cellspacing='2'>
    ...
    <tbody>
      <?php
      // Boucle de sélection
      $req_boucle = $bdd->prepare("SELECT * FROM membres_annonces_ct WHERE id_membre=? ORDER BY id DESC");
      $req_boucle->execute(array($id_oo));
      while ($ligne_boucle = $req_boucle->fetch()) {
       /*  var_dump($ligne_boucle);  */
      ...
      ?>
        <tr class='odd'>
          ...
          <td style='text-align: center; width: 90px;'>
            <?php echo "<a href='/Mes-annonces-ct/modifier/" . $idoneinfos . "' title='Modifier'><span class='uk-icon-file-text'></span></a>"; ?>

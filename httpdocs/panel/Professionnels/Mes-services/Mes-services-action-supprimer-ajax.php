...
if (!empty($_SESSION['4M8e7M5b1R2e8s']) && !empty($user)) {
    ...
    $sql_select_images = $bdd->prepare("SELECT nom_image FROM membres_services_images WHERE id_annonce_service = ? AND id_membre=?");
    ...
    $sql_delete_images = $bdd->prepare("DELETE FROM membres_services_images WHERE id_annonce_service = ?  AND id_membre=?");
    $sql_delete_images->execute(array($idaction, $id_oo));
    $sql_delete_images->closeCursor();

    // Supprimer le produit
    $sql_delete = $bdd->prepare("DELETE FROM membres_services WHERE id = ? AND id_membre = ?");
    $sql_delete->execute(array($idaction, $id_oo));
    $sql_delete->closeCursor();

    $result = array("Texte_rapport" => "Service supprimé.", "retour_validation" => "ok", "retour_lien" => "");

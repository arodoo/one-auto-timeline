...
$nom_fichier = "Mes-services";
$nom_fichier_datatable = "Mes-services-".date('d-m-Y', time())."-$nomsiteweb"; 
?>
<script>
...
<tfoot>
<tr>
<th class='search_table' style='text-align: center;'>SERVICE</th>
...
<?php
// Boucle de sélection
$req_boucle = $bdd->prepare("SELECT * FROM membres_services WHERE id_membre=? ORDER BY id DESC");
...
?>
<tr class='odd'>
<td class='dtr-control' style='text-align: center;'><a href="/Page-service/<?php echo $title_slug; ?>/<?php echo $idoneinfos; ?>" target="_blank"><?php echo $ligne_boucle['nom']; ?></a></td>
<td style='text-align: center;'><?php echo $ligne_select['nom_categorie']; ?></td>
<td style='text-align: center;'><?php if (!empty($ligne_boucle['date'])) { echo date('d-m-Y', $ligne_boucle['date']); } else { echo "--"; } ?></td>
...
<?php echo "<a href='/Mes-services/modifier/".$idoneinfos."' title='Modifier'><span class='uk-icon-file-text'></span></a>"; ?>

    ...
    try {
        ...
        if (empty($statut) || empty($nom) || empty($description) || empty($ville) || empty($id_departement) || empty($id_categorie)) {
            ...
            if ($action == "modifier-action") {
                ...
                $sql_check_images = $bdd->prepare("SELECT COUNT(*) FROM membres_annonces_images WHERE id_annonce_service = ? AND id_membre = ?");
                ...
                $sql_update = $bdd->prepare("UPDATE membres_annonces SET 
                    statut = ?, 
                    nom = ?, 
                    description = ?, 
                    id_categorie = ?, 
                ...
                $sujet = "Annonce modifiée";
                $cleanTitle = createSlug($nom);
                $message_principalone = "<br/>Bonjour,<br/><br/><p>Une annonce a été modifiée avec succès.</p>
                                         <p>Description: $description</p>
                                         <p>Date: " . date('d-m-Y H:i:s') . "</p>
                                         <p>Statut: $statut</p>
                                         <p>Nom du professionnel: $prenom_professional $nom_professional</p>
                                         <p><a href='$nomsiteweb/Page-annonce/$cleanTitle/$idaction'>Lien vers l'annonce</a></p>
                                         L’équipe $nomsiteweb";
            ...
            if ($action == "ajouter-action") {
                $date = time();
                $sql_insert = $bdd->prepare("INSERT INTO membres_annonces (statut, nom, description, id_categorie, ville, id_departement, specialite, meta_description, meta_keyword, mot_cle_1, mot_cle_2, mot_cle_3, mot_cle_4, mot_cle_5, id_membre, pseudo,date, title) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                ...
                $sql_update = $bdd->prepare("UPDATE membres_annonces SET lien = ? WHERE id = ?");
                $sql_update->execute(array($lien, $id_annonce));
                $sql_update->closeCursor();

                // Mettre à jour les images avec le bon id_annonce
                $sql_update_images = $bdd->prepare("UPDATE membres_annonces_images SET id_annonce_service = ? WHERE id_annonce_service = ? AND id_membre=?");
                ...
                $sujet = "Nouvelle annonce ajoutée";
                $cleanTitle = createSlug($nom);
                $message_principalone = "<br/>Bonjour,<br/><br/><p>Une nouvelle annonce a été ajoutée avec succès.</p>
                                         <p>Description: $description</p>
                                         <p>Date: " . date('d-m-Y H:i:s') . "</p>
                                         <p>Statut: $statut</p>
                                         <p>Nom du professionnel: $nom_professional $prenom_professional</p>
                                         <p><a href='$nomsiteweb/Page-annonce/$cleanTitle/$id_annonce'>Lien vers l'annonce</a></p>
                                         L’équipe $nomsiteweb";
                mailsend($vers_mail, $vers_nom, $de_mail, $de_nom, $sujet, $message_principalone);
                echo json_encode($result);
            }

...
$sql = "SELECT nom_image FROM membres_annonces_images WHERE id_membre = ? AND  id_annonce_service = ? AND numero = ?";
...
$sql = "DELETE FROM membres_annonces_images WHERE id_membre = ? AND id_annonce_service = ? AND numero = ?";
$stmt = $bdd->prepare($sql);
$stmt->execute([$id_membre, $id_produit, $photoNum]);
}

// Insérer les informations de la nouvelle image dans la base de données
$sql = "INSERT INTO membres_annonces_images (id_membre, pseudo, id_annonce_service, nom_image, numero) VALUES (?, ?, ?, ?, ?)";
$stmt = $bdd->prepare($sql);

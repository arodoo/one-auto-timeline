...
if (!empty($_SESSION['4M8e7M5b1R2e8s']) && !empty($user)) {
    ...
    $sql_select_images = $bdd->prepare("SELECT nom_image FROM membres_produits_images WHERE id_produit = ? AND id_membre=?");
    ...
    $sql_delete_images = $bdd->prepare("DELETE FROM membres_produits_images WHERE id_produit = ?  AND id_membre=?");
    $sql_delete_images->execute(array($idaction, $id_oo));
    $sql_delete_images->closeCursor();

    // Supprimer le produit
    $sql_delete = $bdd->prepare("DELETE FROM membres_produits WHERE id = ? AND id_membre = ?");

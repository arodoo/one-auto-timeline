        ...
        if (in_array($fileExt, $allowedExts)) {
            ...
            if ($check !== false && strpos($mime, 'image/') === 0) {
                if (move_uploaded_file($fileTmpName, $filePath)) {
                    ...
                    $sql = "SELECT nom_image FROM membres_produits_images WHERE id_membre = ? AND id_produit = ? AND numero = ?";
                    ...
                    if ($existingImage) {
                        ...
                        $sql = "DELETE FROM membres_produits_images WHERE id_membre = ? AND id_produit = ? AND numero = ?";
                        $stmt = $bdd->prepare($sql);
                        $stmt->execute([$id_membre, $id_produit, $photoNum]);
                    }

                    // Insérer les informations de la nouvelle image dans la base de données
                    $sql = "INSERT INTO membres_produits_images (id_membre, pseudo, id_produit, nom_image, numero) VALUES (?, ?, ?, ?, ?)";
                    $stmt = $bdd->prepare($sql);

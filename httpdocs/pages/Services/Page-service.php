...
$req_select_service = $bdd->prepare("SELECT * FROM membres_services WHERE id=?");
$req_select_service->execute(array($_GET['idaction']));
$ligne_select_service = $req_select_service->fetch();
$req_select_service->closeCursor();


//Id de la catégorie du produit
$id_categorie = $ligne_select_service['id_categorie'];
...
$req_select_produit_img = $bdd->prepare("SELECT * FROM membres_services_images WHERE id_annonce_service=?");
...
$req_select_avis_annonce = $bdd->prepare("SELECT COUNT(*) as total_avis, AVG(note) as avg_note FROM membres_avis WHERE id_page=? AND type='service'");
...
<script>
	...
	$(document).ready(function() {
		...
		$('#btn-deposer-avis').on('click', function(event) {
			...
			$.post({
				url: '/pages/Services/Services-avis.php',
				data: data,
				type: 'POST',
				dataType: "json",
				success: function(response) {
...
<script>
	...
	$(document).ready(function() {
		$(document).on("click", "#bouton", function(event) {
			...
			$.post({
				url: '/pages/Services/Services-popup-ajax.php',
				type: 'POST',
				data: formData,
				processData: false,
				contentType: false,
					...
					<div class="col-xl-6 col-lg-6  col-md-6 col-xxl-5 ">
						...
						<div class="tab-content" id="myTabContent">
							<div class="tab-pane fade show active" id="home-tab-pane" role="tabpanel"
								...
								<img class="img-fluid rounded" src="/images/membres/<?= htmlspecialchars($ligne_select_service['pseudo']) ?>/<?= htmlspecialchars($nom_image) ?>" alt="<?= htmlspecialchars($ligne_select_service['title']) ?>">
							</div>
						</div>
					</div>
					...
					<div class="col-xl-6 col-lg-6  col-md-6 col-xxl-7 col-sm-12">
						<div class="product-detail-content">
							...
							<div class="new-arrival-content pr mt-md-0 mt-3">
								<h2><?= htmlspecialchars($ligne_select_service['title']) ?></h2>
								...
								<p>Spécialité : <span class="item"><?= htmlspecialchars($ligne_select_service['specialite']) ?></span> </p>
								<p>Catégorie : <span class="item"><?= htmlspecialchars($ligne_categorie['title']) ?></span> </p>
								<p>Professionnel : <span class="item"><?= htmlspecialchars($ligne_select_service['pseudo']) ?></span></p>
								<p>Secteur géographique : <span class="item"><?= htmlspecialchars($ligne_select_service['ville']) ?></span></p>
								<?php for ($i = 1; $i <= 5; $i++): ?>
									<?php if (!empty($ligne_select_service["mot_cle_$i"])): ?>
										<span class="badge badge-success light"><?= htmlspecialchars($ligne_select_service["mot_cle_$i"]) ?></span>
									<?php endif; ?>
								<?php endfor; ?>
								<p class="text-content"><?= htmlspecialchars($ligne_select_service['description']) ?></p>
				...
				<form id='formulaire-ajouter' method="post" action="#">
					...
					<div class="form-group">
						...
						<select name="departement" id="departement" class="form-control" required>
							...
							foreach ($departements as $departement) {
								$selected = ($_GET['action'] == 'modifier' && $departement['id'] == $ligne_select['departement']) ? "selected" : "";
								echo "<option value='" . $departement['id'] . "' $selected>" . $departement['code'] . " - " . $departement['name'] . "</option>";
							}
							?>
						</select>
					</div>
					<div class="form-group">
						...
						<select name="statut" id="statut" class="form-control">
							<option value="brouillon" <?php if ($_GET['action'] == 'modifier' && $ligne_select['statut'] == "brouillon") echo 'selected'; ?>>Brouillon</option>
							<option value="activé" <?php if ($_GET['action'] == 'modifier' && $ligne_select['statut'] == "activé") echo 'selected'; ?>>Activée</option>
						</select>
					</div>
					-->
					<div class="form-group">
